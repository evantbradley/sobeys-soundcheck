...
const useAudioEngine = () => {
  ...
  // Oscillator Refs for tones
  const oscRef = useRef(null);
  const gainRef = useRef(null);
  
  // Oscillator Refs for unblockable background drone
  ...
  // Conceptual Background Drone (Safari unblockable fix)
  const startBackgroundDrone = () => {
    initAudio();
    stopBackgroundDrone();
    
    const freqs = [150, 183, 221]; // Conceptual static/hum frequencies
  ...
  // Live update background drone volume
  const liveUpdateFilters = (mode) => {
    if (droneGainRef.current) {
      if (mode === 'untreated') droneGainRef.current.gain.setValueAtTime(0.08, audioCtxRef.current.currentTime);
      else if (mode === 'suppression') droneGainRef.current.gain.setValueAtTime(0.015, audioCtxRef.current.currentTime);
      else if (mode === 'active') droneGainRef.current.gain.setValueAtTime(0.002, audioCtxRef.current.currentTime);
    }
  };

  // Muffled and quiet speech test
...
export default function App() {
  ...
  useEffect(() => {
    if (currentFlow === 'instore') {
      ...
      if (step < 11 || step > 12) {
        audio.stopBackgroundDrone();
      }
    }
  }, [step, currentFlow]);

  // Word Test triggering
  useEffect(() => {
    if (currentFlow === 'instore' && step === 11) {
      const timer = setTimeout(() => {
        audio.speakWordTest(testWords[wordStep].w);
      }, 1500); 
      return () => clearTimeout(timer);
    }
  ...
  const progress = currentFlow === 'instore' ? (step / 14) * 100 : currentFlow === 'o2o' ? (step / 5) * 100 : 0;
      ...
      <main className="max-w-6xl w-full flex flex-col justify-center items-center relative z-10 pb-12 mt-12">
        ...
        {currentFlow === 'instore' && step === 10 && (
          <div className="space-y-8 animate-fade-in w-full max-w-5xl flex flex-col items-center">
            ...
            <button onClick={() => { audio.initAudio(); setStep(11); }} className="px-10 py-6 rounded-full bg-[#1B5234] text-[#F9F8F4] text-2xl font-bold hover:bg-[#133c26] active:scale-95 shadow-xl transition-all cursor-pointer mt-8">Experience Solutions</button>
          </div>
        )}
